...
Decision-equivalence evidence is scoped evidence, not a blanket lifecycle proof. The current positive-supported state classes are explicitly modeled `flat_no_dust_no_position`, `open_exposure`, and `reserved_exit_pending`. The research backtest now has a partial `lot_native_simulation_v1` surface that can model deterministic BUY-fill open lots, SELL-submit reserved lots, and partial SELL-fill reserved/open-lot reductions through the shared position-authority shape.

Reports must therefore expose `claims_scope`, `state_coverage_matrix`, and `outcome`. The `lot_native_simulation_v1` surface is partial, not full lifecycle equivalence. Dust-only, non-executable residue, recovery-blocked, and any unsupported or ambiguous lifecycle state still fails closed with `FAIL_CLOSED_UNMODELED_STATE` / `research_model_lacks_lot_native_authority` or a more specific unsupported reason. `FAIL_CLOSED_UNMODELED_STATE` is safe behavior, but it is not profile-transition evidence and is not evidence of full lifecycle equivalence.
