...
If a production-bound run declares stale `attempt_index` or `holdout_reuse_count`, the backtest fails before appending a counted reservation and may append an uncounted `research_attempt_rejected` event. Do not fix this by manually raising counters until a report looks good; inspect the registry and decide whether a new hypothesis or new final-holdout period is required.
...
Aborted events remain counted attempts and are not promotion-permitted. Do not manually edit JSONL rows or hashes.
