...
- Use `uv run bithumb-bot risk-layer-replay --db <runtime.sqlite> --decision-id <id> --json` to recompute persisted strategy, portfolio, and pre-submit risk decision hashes from stored runtime-cycle material. The command opens SQLite read-only and does not call broker APIs or submit orders.
