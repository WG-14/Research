...
def test_embedded_registry_hash_mismatch_is_rejected():
    ...
    with pytest.raises(StrategyCompilationError, match="identity_mismatch"):
        compiled_contract_from_payload(
            payload, expected_registry_hash=sha256_prefixed("different")
        )
...
def test_scenario_fingerprint_contains_compiled_contract_hash():
    ...
    evidence = {
        "decision_stream_hash": digest("d"),
        "execution_request_stream_hash": digest("r"),
        "execution_fill_stream_hash": digest("f"),
        "ledger_stream_hash": digest("l"),
    }
    scenario = {
        "scenario_index": 0,
        "scenario_id": "base",
    ...
    assert (
        fingerprint["compiled_strategy_contract_hash"]
...
def test_scenario_fingerprint_rejects_missing_execution_evidence():
    ...
    with pytest.raises(
        ReproductionContractError, match="execution_evidence is required"
    ):
