...
from market_research.research.audit_trail import (AuditTraceScope, AuditTrailPolicy,
    verify_audit_trail, write_trace_manifest)
...
def test_actual_audit_trace_scope_verifies_completed_run(tmp_path: Path):
    settings = ResearchSettings(data_root=tmp_path / "data", artifact_root=tmp_path / "artifacts",
        report_root=tmp_path / "reports", cache_root=tmp_path / "cache", db_path=tmp_path / "input.db",
        max_workers=1, random_seed=0)
    manager = ResearchPathManager.from_settings(settings, project_root=Path.cwd())
    scope = AuditTraceScope(manager=manager, experiment_id="audit-e2e", manifest_hash="sha256:" + "1" * 64,
        dataset_content_hash="sha256:" + "2" * 64, candidate_id="candidate", scenario_id="base",
        scenario_index=0, split="validation")
    run = run_common_simulation_backtest(plugin=resolve_research_strategy("noop_baseline"),
        dataset=_dataset(), parameter_values={}, fee_rate=0, slippage_bps=0,
        context=BacktestRunContext(audit_trace=scope))
    manifest = write_trace_manifest(manager=manager, experiment_id="audit-e2e",
        manifest_hash="sha256:" + "1" * 64, dataset_content_hash="sha256:" + "2" * 64,
        trace_indexes=[run.audit_trace_index], policy=AuditTrailPolicy(mode="complete_external",
            decisions_required=True, equity_required=True, executions_required=False))
    verified = verify_audit_trail(manager=manager, trace_manifest_path_value=manifest["trace_manifest_path"]
        if "trace_manifest_path" in manifest else manager.data_dir() / "derived/research/audit-e2e/trace_manifest.json")
