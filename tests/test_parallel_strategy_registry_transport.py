...
def build_importable_custom_plugin():
    ...
    return ResearchStrategyPlugin(
        ...
        optional_data=(),
        event_builder=_custom_events,
        decision_contract_version="v1",
        diagnostics_namespace="custom",
...
def test_registry_descriptor_round_trip_and_deterministic_order():
    ...
    assert [item["strategy_name"] for item in descriptor["plugins"]] == sorted(
        registry.plugins
    )
    assert (
        reconstruct_strategy_registry(descriptor).content_hash == registry.content_hash
    )
...
def test_parallel_custom_importable_plugin_runs_and_unreconstructable_fails_closed():
    ...
    assert (
        reconstruct_strategy_registry(registry.descriptor())
        .resolve("transport_custom")
        .name
        == "transport_custom"
    )
    with ProcessPoolExecutor(max_workers=1, mp_context=mp.get_context("spawn")) as pool:
        assert (
            pool.submit(_reconstruct_hash, registry.descriptor()).result(timeout=30)
