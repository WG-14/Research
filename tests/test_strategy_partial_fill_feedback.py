...
def _observing_plugin(observed):
    ...
    def factory(**kwargs):
        ...
        class Runtime:
            def initialize(self, context): return delegate.initialize(context)
...
def _run(model, observed):
    ...
    compiled = StrategyCompiler(registry).compile(strategy_name=plugin.name,
        raw_parameters={"THRESHOLD_CLOSE_ABOVE": 100}, fee_rate=.001, slippage_bps=10)
    return run_common_simulation_backtest(plugin=plugin, dataset=_data(),
        parameter_values={"THRESHOLD_CLOSE_ABOVE": 100}, fee_rate=.001, slippage_bps=10,
        execution_model=model, execution_timing_policy=ExecutionTimingPolicy(fill_reference_policy="next_candle_open"),
        portfolio_policy=legacy_research_portfolio_policy(), compiled_contract=compiled)


def test_strategy_sees_actual_average_cost_after_partial_fill():
    observed = []
    run = _run(StressExecutionModel(.001, 10, partial_fill_rate=1, partial_fill_fraction=.5, seed=1), observed)
    ...
    assert filled.average_cost == run.ledger_entries[0].cost_basis_after / run.ledger_entries[0].asset_qty_after


def test_fill_failure_is_visible_to_next_strategy_decision():
    observed = []
    _run(StressExecutionModel(.001, 10, order_failure_rate=1, seed=1), observed)
    assert any(view.last_execution_status == "failed" and view.filled_position_qty == 0 for view in observed)
