...
def test_plugin_contract_binds_every_behavior_hook():
    ...
    assert {"parameter_materializer", "runtime_factory", "event_builder_compatibility",
            "exit_policy_materializer", "exit_decision_builder", "data_requirements_builder"} <= set(hooks)
