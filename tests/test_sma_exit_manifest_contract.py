...
def test_each_sma_exit_rule_parses_and_compiles_from_manifest(rules, parameter, value):
    ...
    contract = StrategyCompiler(builtin_strategy_registry()).compile(
        ...
        raw_parameters={
            key: values[0] for key, values in manifest.parameter_space.items()
        },
        fee_rate=0.0,
        slippage_bps=0.0,
    )
...
def test_exit_threshold_without_matching_rule_fails_closed():
    with pytest.raises(
        ManifestValidationError, match="does not include edge_invalidation"
    ):
        parse_builtin_manifest(
            _sma_payload("stop_loss", STRATEGY_EXIT_MIN_EDGE_RATIO=0.01)
        )


def test_take_profit_and_edge_invalidation_trigger_independently():
    position = ResearchPosition(
        cash=0, asset_qty=1, entry_price=100, entry_ts=0, sellable_qty=1
    )
    take = evaluate_sma_exit_policy(
        policy={"rules": ["take_profit"], "take_profit": {"take_profit_ratio": 0.05}},
        position=position,
        candle_ts=60_000,
        market_price=106,
        exit_signal="HOLD",
    )
    edge = evaluate_sma_exit_policy(
        policy={
            "rules": ["edge_invalidation"],
            "edge_invalidation": {"min_edge_ratio": 0.02},
        },
        position=position,
        candle_ts=60_000,
        market_price=100,
        exit_signal="HOLD",
        feature_state={"gap_ratio": 0.01},
    )
