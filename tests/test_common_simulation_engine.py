...
def _dataset() -> DatasetSnapshot:
    return DatasetSnapshot(
        ...
        tuple(
            Candle(i * 60_000, 100 + i, 101 + i, 99 + i, 100 + i, 1.0) for i in range(5)
        ),
    )
...
def test_reference_failure_does_not_create_portfolio_fill():
    ...
    failed = run_common_simulation_backtest(
        plugin=resolve_research_strategy("buy_and_hold_baseline"),
        dataset=_dataset(),
...
def test_portfolio_policy_is_the_single_buy_sizing_authority():
    ...
    sizing = replace(
        legacy_research_portfolio_policy().position_sizing, buy_fraction=0.5
    )
