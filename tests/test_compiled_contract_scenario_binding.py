...
def _compiled(fee: float, slippage: float):
    ...
    return StrategyCompiler(registry).compile(
        ...
        fee_rate=fee, slippage_bps=slippage,
    )
...
def test_train_validation_holdout_share_candidate_scenario_contract():
    ...
    runs = [run_common_simulation_backtest(
        plugin=plugin, dataset=_dataset(), parameter_values={"SMA_SHORT": 1, "SMA_LONG": 2},
        fee_rate=.001, slippage_bps=10, compiled_contract=compiled, registry=registry,
    ) for _split in ("train", "validation", "final_holdout")]
    assert all(run.compiled_strategy_contract is compiled for run in runs)
    assert {run.compiled_strategy_contract_hash for run in runs} == {compiled.compiled_contract_hash}


def test_each_scenario_binds_its_own_compiled_contract():
    base = _compiled(.001, 10)
    stress = _compiled(.002, 25)
