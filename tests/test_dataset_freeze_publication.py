...
def test_freeze_is_idempotent_for_identical_input(tmp_path) -> None:
    ...
    first = freeze_sqlite_candles_dataset(
        source_provenance=TEST_SOURCE_PROVENANCE,
        source_db=source,
        market="KRW-BTC",
        interval="1m",
    ...
    second = freeze_sqlite_candles_dataset(
        source_provenance=TEST_SOURCE_PROVENANCE,
        source_db=source,
        market="KRW-BTC",
        interval="1m",
...
@pytest.mark.parametrize(
    ...
    (
        "during_db_write",
        "during_db_creation",
        "during_manifest_creation",
        "after_verification_before_rename",
        "during_final_publication",
    ),
)
...
def test_concurrent_conflicting_publication_fails(tmp_path) -> None:
    ...
    assert any(
        ...
        in {
            ...
            "existing_artifact_invalid_or_tampered",
        }
        for result in results
    )


def test_existing_content_hash_conflict_preserves_public_error_contract(
    tmp_path,
) -> None:
    source = _source(tmp_path)
    frozen = freeze_sqlite_candles_dataset(
