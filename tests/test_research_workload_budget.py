...
from bithumb_bot.research.experiment_manifest import ResearchResourceLimits
...
def test_research_workload_budget_script_fails_oversized_synthetic_estimate(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "max_artifact_bytes": 64 * 1024 * 1024 + 1,
                "max_audit_stream_bytes": 1,
                "max_artifact_file_count": 501,
            }
        ),
        encoding="utf-8",
    )
    ...
    assert "estimated_tick_events exceeded: observed=25001 limit=25000" in output
    assert "estimated_audit_stream_rows exceeded: observed=1 limit=0" in output
