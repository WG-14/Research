...
def test_retention_limit_preserves_full_canonical_hash():
    ...
    a = run_common_simulation_backtest(
        plugin=plugin,
        dataset=_dataset(),
        parameter_values={},
        fee_rate=0,
    ...
    b = run_common_simulation_backtest(
        plugin=plugin,
        dataset=_dataset(),
        parameter_values={},
        fee_rate=0,
        ...
        context=BacktestRunContext(
            resource_limits=BacktestResourceLimits(
                max_decisions_retained=1, max_equity_points_retained=1
            )
        ),
    )
    assert a.decision_stream_hash == b.decision_stream_hash
    assert (
        a.execution_event_summary["ledger_stream_hash"]
        == b.execution_event_summary["ledger_stream_hash"]
    )
...
def test_retained_projection_still_validates_authoritative_lineage():
    run = run_common_simulation_backtest(
        ...
        slippage_bps=0,
        context=BacktestRunContext(
            resource_limits=BacktestResourceLimits(max_decisions_retained=1)
        ),
    )
