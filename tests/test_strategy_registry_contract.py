...
def test_registry_hash_changes_when_plugin_contract_changes():
    ...
    assert (
        StrategyRegistry.build((plugin,)).content_hash
        != StrategyRegistry.build((changed,)).content_hash
    )
...
def test_selected_execution_hash_ignores_unrelated_catalog_additions():
    ...
    assert expanded.execution_scope_hash(
        selected.name
    ) == registry.execution_scope_hash(selected.name)
...
def test_registry_detects_stale_plugin_contract():
    ...
    try:
        with pytest.raises(
            StrategyRegistryError, match="stale_research_strategy_contract"
        ):
