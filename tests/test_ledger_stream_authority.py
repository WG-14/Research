...
def _run():
    return run_common_simulation_backtest(plugin=resolve_research_strategy("buy_and_hold_baseline"),
...
def test_every_effective_mutating_fill_has_one_ledger_entry():
    run = _run()
    effective = {fill.fill_id for fill in run.fills
...
def test_pending_after_dataset_fill_has_explicit_pending_evidence():
    ...
    run = run_common_simulation_backtest(plugin=resolve_research_strategy("buy_and_hold_baseline"),
        dataset=data, parameter_values={"BUY_HOLD_BUY_INDEX": 4}, fee_rate=0, slippage_bps=0,
        execution_timing_policy=ExecutionTimingPolicy(fill_reference_policy="first_orderbook_after_decision",
                                                       max_quote_wait_ms=3000))
    assert len(run.fills) == 1 and not run.ledger_entries
    pending = next(trade for trade in run.trades if trade.get("fill_id") == run.fills[0].fill_id)
