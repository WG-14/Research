...
def test_noop_research_kernel_matches_fixed_legacy_behavior_golden() -> None:
    ...
    assert [item["candle_ts"] for item in result.decisions] == [
        60_000,
        120_000,
        180_000,
        240_000,
    ]
    assert [item["decision_ts"] for item in result.decisions] == [
        120_000,
        180_000,
        240_000,
        300_000,
    ...
    assert [
        item["strategy_diagnostics"]["hold_decision_count"] for item in result.decisions
    ] == [1, 2, 3, 4]
    assert result.trades == ()
    assert {
        key: result.execution_event_summary[key]
        for key in (
    ...
    assert result.execution_event_summary["execution_request_stream_hash"].startswith(
        "sha256:"
    )


def test_noop_event_defaults_and_negative_start_index_preserve_legacy_semantics() -> (
    None
):
    events = build_noop_baseline_events(
        dataset=_dataset(),
    ...
    assert [event.feature_snapshot["candle_index"] for event in events] == [
        0,
        1,
        2,
        3,
