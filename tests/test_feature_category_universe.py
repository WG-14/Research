...
from bithumb_bot.research.feature_provider_registry import FeatureProviderSpec, feature_provider_spec_for_name
...
def test_category_universe_creates_zero_count_bucket_for_missing_category() -> None:
    ...
    assert by_bucket["category:trend_up"].count == 1
...
def test_unknown_category_value_emits_warning() -> None:
    ...
    unknown = next(metric for metric in metrics if metric.bucket_id == "category:unknown_regime")
...
def test_regime_provider_spec_declares_category_universe() -> None:
    ...
    assert spec.category_universe
    assert "uptrend_normal_vol_volume_normal" in spec.category_universe


def test_category_universe_is_included_in_report_feature_provider_specs(tmp_path) -> None:
    spec = feature_provider_spec_for_name("regime")
    ...
    assert "category_universe" in report["feature_provider_specs"][0]
...
def test_report_content_hash_changes_when_category_universe_changes(tmp_path) -> None:
    spec = feature_provider_spec_for_name("regime")
    changed = replace(spec, category_universe=spec.category_universe + ("test_new_regime",))
    ...
    second = write_forward_diagnostics_report(
        ...
        manifest=_manifest(),
        result=_result(feature_provider_specs=(changed,)),
    )
