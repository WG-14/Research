...
def test_candidate_profile_recursively_excludes_runtime_and_path_observations() -> None:
        ...
        "scenario_results": [
            {
                ...
                "validation_resource_usage": {"runtime_seconds": 0.25, "final_cash": 100.0},
            }
        ],
    ...
    relocated["scenario_results"][0]["validation_resource_usage"]["runtime_seconds"] = 99.0
    ...
    assert first == second
    assert first["scenario_results"][0]["validation_resource_usage"] == {"final_cash": 100.0}
    assert sha256_prefixed(first) == sha256_prefixed(second)
    assert candidate["scenario_results"][0]["detail_artifact_hash"] == "sha256:" + "2" * 64

    changed_evidence = deepcopy(candidate)
    changed_evidence["scenario_results"][0]["validation_resource_usage"]["final_cash"] = 101.0
    assert build_candidate_profile(changed_evidence) != first


def test_report_logical_hash_excludes_nested_lineage_runtime_but_keeps_evidence() -> None:
    report = {
        "manifest_hash": "sha256:" + "1" * 64,
        "lineage": {
            "manifest_hash": "sha256:" + "1" * 64,
            "manifest_path": "/inputs-a/manifest.json",
            "normalized_command_args": {"manifest": "/inputs-a/manifest.json", "run_id": "RUN-a"},
        ...
        "candidates": [
            {
                ...
                "validation_resource_usage": {"runtime_seconds": 0.1, "final_cash": 100.0},
            }
        ],
    }
    ...
    rerun["lineage"].update(
        {
            ...
            "normalized_command_args": {"manifest": "/inputs-b/manifest.json", "run_id": "RUN-b"},
