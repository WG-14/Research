...
def test_materialize_requires_verified_artifact_and_reuses_verification_within_run(
    tmp_path,
) -> None:
    ...
    assert (
        context.resolve_verified(adapter, ref, DatasetResolutionContext()) is verified
    )
    assert (
    ...
    snapshot = adapter.materialize(
        verified, DatasetSliceQuery("KRW-BTC", "1m", 1, 1, "train", "test", {})
    )
    assert len(snapshot.candles) == 1
    with pytest.raises(ValueError, match="outside"):
        adapter.materialize(
            verified, DatasetSliceQuery("KRW-BTC", "1m", 0, 1, "train", "test", {})
        )
...
def test_real_backtest_verifies_once_and_materializes_selection_splits(
...
def test_real_walk_forward_verifies_once_and_materializes_every_split(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
