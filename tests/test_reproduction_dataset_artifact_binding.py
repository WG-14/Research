...
def _recompute_stable_fingerprint_hash(fingerprint: dict[str, object]) -> None:
    material = {key: value for key, value in fingerprint.items() if key != "stable_fingerprint_hash"}
...
def _split(fingerprint: dict[str, object], split_name: str) -> dict[str, object]:
    return next(
        split for split in fingerprint["dataset_split_hashes"]
...
def test_artifact_and_split_drift_report_distinct_reproduction_paths(tmp_path) -> None:
    ...
    assert {
        field: changed_split[field]
        for field in original_artifact
    } == original_artifact
    assert any(
...
def test_missing_artifact_evidence_rejects_receipt_creation(tmp_path, field: str) -> None:
    report, manifest, _ = _valid_report_and_fingerprint(tmp_path)
    incomplete = copy.deepcopy(report)
    incomplete["dataset_splits"]["train"].pop(field)

    with pytest.raises(ReproductionContractError, match=rf"dataset_splits\.train\.{field} is required"):
        create_reproduction_receipt(
            report=incomplete,
            manifest=manifest,
