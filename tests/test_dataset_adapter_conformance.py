...
from market_research.research.datasets.registry import (
...
def _frozen_fixture(tmp_path: Path) -> tuple[object, DatasetVerificationResult]:
    ...
    snapshot = adapter.materialize(
        verified, query=DatasetSliceQuery("KRW-BTC", "1m", 1, 2, "train", "frozen", {})
    )
    return snapshot, adapter.verify_snapshot(
        snapshot=snapshot, context=DatasetLoadContext()
    )
...
def _sqlite_fixture(_: Path) -> tuple[object, DatasetVerificationResult]:
    ...
    return snapshot, SQLiteCandleAdapter().verify_snapshot(
        snapshot=snapshot, context=DatasetLoadContext()
    )
...
def test_conformance_fixture_exists_for_every_registered_adapter() -> None:
    assert set(default_dataset_adapter_registry().sources()) == set(
        CONFORMANCE_FIXTURE_FACTORIES
    )


@pytest.mark.parametrize("source", default_dataset_adapter_registry().sources())
def test_all_registered_adapters_pass_verification_contract(
    ...
    if source == "frozen_sqlite_candles":
        ...
        assert result.overall_status in {
            ...
            VerificationStatus.DERIVED_FROM_SNAPSHOT,
        }
...
def test_sqlite_adapter_does_not_claim_verified_without_full_scan(
    tmp_path: Path,
) -> None:
...
def test_mismatch_fails_before_strategy_execution(
    ...
    class SpyEvaluator:
        def evaluate(self, *args, **kwargs):
            ...
            raise AssertionError(
                "strategy runner must not execute after verification mismatch"
            )

    monkeypatch.setattr(SQLiteCandleAdapter, "verify_snapshot", mismatch)
    with pytest.raises(
        Exception, match="dataset_verification_mismatch_before_strategy_execution"
    ):
...
def test_verification_hash_relationships_are_constructor_enforced(
    status, expected, actual, message
) -> None:
