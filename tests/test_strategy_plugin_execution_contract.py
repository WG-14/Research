...
def test_all_builtin_plugins_use_common_execution_authority():
    assert {plugin.execution_authority for plugin in list_research_strategies()} == {"common_simulation_engine"}
...
def test_plugin_cannot_register_custom_backtest_runner():
    ...
    with pytest.raises(ValueError, match="custom_execution_authority"):
        ResearchStrategyPlugin(name="test", version="1", spec=plugin.spec, required_data=(), optional_data=(), event_builder=plugin.event_builder, decision_contract_version="1", diagnostics_namespace="test", execution_authority="custom_runner")
...
def test_plugin_contract_hash_binds_event_builder_and_execution_authority():
    ...
    assert payload["execution_authority"] == "common_simulation_engine" and payload["event_builder_qualname"]
