...
def _result():
    ...
    candidate = {"parameter_candidate_id": "candidate-1", "primary_scenario_id": "base",
                 "parameter_values": {}, "parameter_values_raw": {},
    ...
    if not confirmation.get("experiment_registry_path"):
        reservation = reserve_research_attempt(
            ...
            base_payload={
                ...
                "selection_artifact_hash": report["selection_artifact"][
                    "content_hash"
                ],
                "selected_candidate_id": "candidate-1",
        ...
        confirmation.update(
            {
                ...
                "experiment_registry_completion_row_hash": completion[
                    "row_hash"
                ],
...
def test_package_contains_execution_and_ledger_contract_hashes(monkeypatch, tmp_path):
    ...
    assert package["decision_contract_version"] == report["strategy_spec"][
        "decision_contract_version"
    ]
    assert package["risk_policy"] == report["risk_policy"]
    assert build_strategy_research_package(
        report, approval=approval, manager=manager
