...
def test_manifest_parser_requires_explicit_registry():
    with pytest.raises(
        ManifestValidationError, match="authoritative_manifest_registry_required"
    ):
        parse_manifest({})
...
def test_custom_registry_flows_through_manifest_and_simulation(tmp_path):
    ...
    assert compiled.strategy_registry_hash == registry.execution_scope_hash(
...
def test_manifest_registry_hash_must_match_runtime_registry_hash(tmp_path):
    ...
    registry_b = StrategyRegistry.build(
        (replace(plugin, version=plugin.version + ".different"),)
    )
    with pytest.raises(
        ResearchValidationError,
        match="manifest_runtime_strategy_registry_hash_mismatch",
    ):
        run_research_backtest(
            manifest=manifest,
            db_path=None,
...
def _custom_plugin() -> ResearchStrategyPlugin:
    rules = StrategyRuleSpec(
        ...
        edge_invalidation=StrategyRuleDeclaration(
            "edge_invalidation", "Disabled.", "never"
        ),
        ...
        position_sizing=StrategyRuleDeclaration(
            "no_position", "No allocation.", "always"
        ),
    )
    spec = StrategySpec(
        "fixture_custom_alpha",
        "fixture.v1",
        ("WINDOW",),
        ("WINDOW",),
        ("WINDOW",),
        ...
        (StrategyParameterSchema("WINDOW", "int", min_value=1),),
        rules,
    )
    return ResearchStrategyPlugin(
        ...
        optional_data=(),
        event_builder=lambda **_values: (),
        decision_contract_version=spec.decision_contract_version,
        diagnostics_namespace="fixture",
    )


def test_true_custom_strategy_name_parses_and_compiles_without_global_catalog(
    ...
    monkeypatch.setattr(
        "market_research.research.strategy_spec.strategy_spec_for_name",
...
def test_unrelated_strategy_addition_does_not_change_existing_compiled_contract():
    ...
    unrelated = replace(
        plugin, name="unrelated_fixture", version="unrelated_fixture.v1"
    )
