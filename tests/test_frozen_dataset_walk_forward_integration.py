...
from market_research.research.data_plane import rolling_walk_forward_windows, walk_forward_payload
...
def test_walk_forward_windows_stop_at_validation_end(tmp_path) -> None:
    ...
    assert windows
    assert all(window["train"].end <= manifest.dataset.split.validation.end for window in windows)
    assert all(window["test"].end <= manifest.dataset.split.validation.end for window in windows)
...
def test_walk_forward_windows_are_invariant_to_final_holdout_range(tmp_path) -> None:
    ...
    window_payload = [{name: value.as_dict() for name, value in window.items()} for window in windows]
    extended_payload = [{name: value.as_dict() for name, value in window.items()} for window in extended_windows]
