...
def test_sma_opposite_cross_noise_band_does_not_trigger_sell():
    decision = evaluate_sma_exit_policy(
        policy={
            "rules": ["opposite_cross"],
            "opposite_cross": {
                "min_take_profit_ratio": 0.01,
                "small_loss_tolerance_ratio": 0.01,
...
def test_sma_noise_band_creates_no_sell_intent_request_fill_or_ledger():
    ...
    class Runtime:
        ...
        def on_market_event(self, market, portfolio, state):
            ...
            event = ResearchDecisionEvent(
                ...
                raw_signal="BUY" if candle.ts == 0 else "SELL",
                final_signal="BUY" if candle.ts == 0 else "HOLD",
                entry_signal="BUY" if candle.ts == 0 else "HOLD",
                exit_signal="HOLD" if candle.ts == 0 else "SELL",
                reason="noise-band-fixture",
                feature_snapshot={},
    ...
    parameters = {
        ...
        "STRATEGY_EXIT_SMALL_LOSS_TOLERANCE_RATIO": 0.01,
    }
    ...
    assert any(
        item["source"] == "strategy_exit_callback" and item["triggered"] is False
