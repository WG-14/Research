...
from .validation_evidence_verifier import verify_validation_candidate_execution_evidence


VALIDATION_ARTIFACT_GRADE = "validation_candidate"
DIAGNOSTIC_ARTIFACT_GRADE = "diagnostic_only"
VALIDATION_AUTHORITY_PLANE = "typed_execution_plan_bundle"
VALIDATION_EXECUTION_EVIDENCE_SOURCE = "typed_execution_plan_bundle"
VALIDATION_NEXT_ACTION = "regenerate_with_typed_execution_authority"
...
class ValidationArtifactProvenance:
    authority_plane: str
    ...
    research_profile_hash: str
    compatibility_fallback: bool
    legacy_context_planning_used: bool
    runtime_replay_planning_error: str | None
    artifact_grade: str
    validation_rejection_reason: str

    @classmethod
    def from_payload(cls, payload: dict[str, Any]) -> "ValidationArtifactProvenance":
        return cls(
            ...
            research_profile_hash=str(payload.get("research_profile_hash") or ""),
            ...
            validation_rejection_reason=str(payload.get("validation_rejection_reason") or ""),
        )
...
@dataclass(frozen=True)
class ValidationProvenanceValidation:
    ok: bool
    ...
    def create_from_typed_bundle(
        ...
        *,
        ...
        if int(canonical_decision_v2.get("decision_contract_version") or 0) < 2:
            raise ValueError("validation_artifact_requires_canonical_v2")
        ...
        payload.update(
            {
                ...
                "authority_plane": VALIDATION_AUTHORITY_PLANE,
                "decision_authority_source": str(
                    payload.get("decision_authority_source")
                    or "DecisionEnvelope.strategy_decision"
                ),
                "execution_evidence_source": VALIDATION_EXECUTION_EVIDENCE_SOURCE,
                ...
                "runtime_replay_planning_error": "",
                "artifact_grade": VALIDATION_ARTIFACT_GRADE,
                "validation_grade": submit_payload is not None,
                "validation_rejection_reason": "",
            }
        )
        validation = validate_validation_artifact(payload)
        if not validation.ok:
            raise ValueError(
                "validation_artifact_invalid:" + ",".join(validation.reason_codes)
            )
...
def validate_validation_artifact_provenance(
    payload: dict[str, Any],
) -> ValidationProvenanceValidation:
    provenance = ValidationArtifactProvenance.from_payload(payload)
    failures = validation_provenance_failure_codes(provenance)
    failures.extend(_typed_evidence_failure_codes(payload, provenance))
    failures.extend(
        "canonical_" + reason
        for reason in verify_validation_candidate_execution_evidence(payload).reason_codes
    )
    return ValidationProvenanceValidation(
        ok=not failures,
        reason_codes=tuple(sorted(set(failures))),
        recommended_next_action="none" if not failures else VALIDATION_NEXT_ACTION,
        provenance=provenance,
    )


def validate_validation_artifact(payload: dict[str, Any]) -> ValidationProvenanceValidation:
    """Hard gate for validation artifacts: canonical v2 plus verified typed evidence."""
    failures: list[str] = []
    try:
        ...
        version = 0
    if version < 2:
        failures.append("canonical_validation_legacy_contract_version")
    validation = validate_validation_artifact_provenance(payload)
...
def validation_provenance_failure_codes(
    provenance: ValidationArtifactProvenance,
) -> list[str]:
    failures: list[str] = []
    if provenance.compatibility_fallback:
        failures.append("canonical_validation_compatibility_fallback")
    if provenance.legacy_context_planning_used:
        failures.append("canonical_validation_legacy_context_planning")
    if not provenance.execution_plan_bundle_present:
        failures.append("canonical_validation_execution_plan_bundle_missing")
    if not _valid_sha256_hash(provenance.execution_plan_bundle_hash):
        failures.append("canonical_validation_execution_plan_bundle_hash_missing")
    if not provenance.typed_execution_summary_present:
        failures.append("canonical_validation_typed_execution_summary_missing")
    if not _valid_sha256_hash(provenance.execution_summary_hash):
        failures.append("canonical_validation_execution_summary_hash_missing")
    if not _valid_sha256_hash(provenance.execution_submit_plan_hash):
        failures.append("canonical_validation_execution_submit_plan_hash_missing")
    if not _valid_sha256_hash(provenance.runtime_decision_request_hash):
        failures.append("canonical_validation_runtime_decision_request_hash_missing")
    if not _valid_sha256_hash(provenance.runtime_strategy_set_manifest_hash):
        failures.append("canonical_validation_runtime_strategy_set_manifest_hash_missing")
    if not _valid_sha256_hash(provenance.research_profile_hash):
        failures.append("canonical_validation_research_profile_hash_missing")
    if provenance.decision_authority_source.strip() in LEGACY_DECISION_AUTHORITY_SOURCES:
        failures.append("canonical_validation_legacy_context_authority")
    if provenance.runtime_replay_planning_error:
        failures.append("canonical_validation_runtime_replay_planning_error")
    if provenance.execution_evidence_source != VALIDATION_EXECUTION_EVIDENCE_SOURCE:
        failures.append("canonical_validation_typed_execution_provenance_missing")
    if provenance.authority_plane != VALIDATION_AUTHORITY_PLANE:
        failures.append("canonical_validation_typed_authority_plane_missing")
    if provenance.artifact_grade != VALIDATION_ARTIFACT_GRADE:
        failures.append("canonical_validation_artifact_grade_not_validation")
    if provenance.validation_rejection_reason.strip():
        failures.append("canonical_validation_rejection_reason_present")
...
def _canonical_decision_failure_codes(payload: dict[str, Any]) -> list[str]:
    ...
    if str(payload.get("policy_materialization_mode") or "") == "research_exploratory":
        failures.append("canonical_validation_research_exploratory_materialization")
    if payload.get("research_comparable") is False:
        failures.append("canonical_validation_research_comparable_false")
    if payload.get("allow_execution_compatibility_fallback") is True:
        failures.append("canonical_validation_execution_compatibility_fallback")
    for field_name in (
        "policy_input_hash",
        ...
        if not _valid_sha256_hash(str(payload.get(field_name) or "")):
            failures.append(f"canonical_validation_{field_name}_missing")
    if not (
        ...
        failures.append("canonical_validation_market_feature_hash_missing")
    if not str(payload.get("snapshot_projector_version") or "").strip():
        failures.append("canonical_validation_snapshot_projector_version_missing")
    provenance = payload.get("strategy_evaluation_provenance")
    if not isinstance(provenance, dict):
        failures.append("canonical_validation_strategy_evaluation_provenance_missing")
    elif provenance.get("decision_boundary") != "StrategyDecisionService.evaluate":
        failures.append("canonical_validation_strategy_evaluation_boundary_invalid")
    return failures
...
def _typed_evidence_failure_codes(
    ...
    if not isinstance(bundle_evidence, dict) or not isinstance(summary_evidence, dict):
        failures.append("canonical_validation_forged_or_unverified_typed_evidence")
        return failures
    if bundle_evidence.get("compatibility_fallback") is True:
        failures.append("canonical_validation_compatibility_fallback")
    if bundle_evidence.get("validation_grade") is False:
        failures.append("canonical_validation_bundle_not_validation_grade")
    if str(bundle_evidence.get("artifact_grade") or VALIDATION_ARTIFACT_GRADE) != VALIDATION_ARTIFACT_GRADE:
        failures.append("canonical_validation_artifact_grade_not_validation")
    if str(bundle_evidence.get("authority_plane") or VALIDATION_AUTHORITY_PLANE) != VALIDATION_AUTHORITY_PLANE:
        failures.append("canonical_validation_typed_authority_plane_missing")
    if (
        str(bundle_evidence.get("execution_evidence_source") or VALIDATION_EXECUTION_EVIDENCE_SOURCE)
        != VALIDATION_EXECUTION_EVIDENCE_SOURCE
    ):
        failures.append("canonical_validation_typed_execution_provenance_missing")
    if bundle_evidence.get("live_authoritative") is True:
        failures.append("canonical_validation_research_bundle_claims_live_authority")
    ...
    if isinstance(submit_evidence, dict):
        if str(submit_evidence.get("authority_label") or "") != "ExecutionSubmitPlan.final_payload.v1":
            failures.append("canonical_validation_dict_only_submit_evidence_not_authority")
        try:
        ...
        if submit_evidence.get("compatibility_fallback") is True:
            failures.append("canonical_validation_compatibility_fallback")
        if submit_evidence.get("validation_grade") is False:
            failures.append("canonical_validation_submit_plan_not_validation_grade")
        if str(submit_evidence.get("artifact_grade") or VALIDATION_ARTIFACT_GRADE) != VALIDATION_ARTIFACT_GRADE:
            failures.append("canonical_validation_artifact_grade_not_validation")
...
def payload_has_validation_provenance_markers(payload: dict[str, Any]) -> bool:
    return any(
        key in payload
        for key in (
            "authority_plane",
...
def verify_validation_provenance_artifact_file(path: str | Path) -> dict[str, Any]:
    resolved = Path(path).resolve()
    try:
        payload = json.loads(resolved.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        validation = validate_validation_artifact({}).as_dict()
        validation.update(
            {
                "ok": False,
                "reason_codes": sorted(
                    set(list(validation["reason_codes"]) + ["validation_artifact_unreadable"])
                ),
                "recommended_next_action": VALIDATION_NEXT_ACTION,
    ...
    if not isinstance(payload, dict):
        ...
        validation["reason_codes"] = sorted(
            set(list(validation["reason_codes"]) + ["validation_artifact_schema_not_object"])
        )
        validation["recommended_next_action"] = VALIDATION_NEXT_ACTION
        validation["artifact_path"] = str(resolved)
        return validation
    validation = validate_validation_artifact(payload).as_dict()
    validation["artifact_path"] = str(resolved)
    return validation
