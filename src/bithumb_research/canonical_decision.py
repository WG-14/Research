...
from .validation_provenance import (
    VALIDATION_ARTIFACT_GRADE,
    VALIDATION_AUTHORITY_PLANE,
    VALIDATION_EXECUTION_EVIDENCE_SOURCE,
    build_typed_no_submit_proof,
    validate_validation_artifact as validate_validation_artifact_provenance_gate,
)
...
VALIDATION_REQUIRED_CANONICAL_FIELDS = (
    "decision_contract_version",
    "strategy_name",
    "strategy_decision_contract_version",
    "profile_content_hash",
...
LEGACY_VALIDATION_REQUIRED_CANONICAL_FIELDS_V1 = (
    "decision_contract_version",
    "strategy_contract_version",
    "strategy_name",
    "profile_content_hash",
...
VALIDATION_REQUIRED_ONE_OF_CANONICAL_FIELDS = (("signal_timestamp", "candle_ts"),)
VALIDATION_PROVENANCE_REQUIRED_CANONICAL_FIELDS = (
    "runtime_decision_request_hash",
    "runtime_strategy_set_manifest_hash",
    "research_profile_hash",
    "execution_plan_bundle_hash",
...
VALIDATION_TYPED_EXECUTION_EVIDENCE_SOURCE = VALIDATION_EXECUTION_EVIDENCE_SOURCE
...
def validate_canonical_decision_payload(
    ...
    *,
    ...
    require_validation_provenance: bool = True,
    ...
    required_fields = (
        ...
        else VALIDATION_REQUIRED_CANONICAL_FIELDS
    )
    if not require_validation_provenance:
        required_fields = tuple(
            field
            for field in required_fields
            if field not in VALIDATION_PROVENANCE_REQUIRED_CANONICAL_FIELDS or field not in payload
        )
    for field in required_fields:
        if _canonical_required_missing(normalized.get(field)):
            missing.append(field)
    for group in VALIDATION_REQUIRED_ONE_OF_CANONICAL_FIELDS:
    ...
    if validation_grade:
        if int(normalized.get("decision_contract_version") or 0) < CANONICAL_DECISION_CONTRACT_VERSION:
            ...
            reason_codes.extend(
                [
                    "canonical_validation_legacy_contract_version",
                    "canonical_decision_incomplete",
                ]
            )
        if require_validation_provenance:
            provenance_failures = _validation_artifact_provenance_failures(payload, normalized)
    ...
    is_validation_grade = bool(complete)
    if validation_grade and not is_validation_grade:
        reason_codes.append("canonical_decision_not_validation_grade")
    return CanonicalDecisionValidation(
        canonical_schema_present=True,
        canonical_schema_complete=complete,
        validation_grade=is_validation_grade,
...
def validate_validation_artifact(payload: dict[str, Any]) -> CanonicalDecisionValidation:
    """Strict validation-grade validation for canonical runtime/research artifacts."""
    return validate_canonical_decision_payload(payload, validation_grade=True)


def _validation_artifact_provenance_failures(
    payload: dict[str, Any],
    ...
    validation = validate_validation_artifact_provenance_gate(provenance_payload)
    reason_to_field = {
        "canonical_validation_compatibility_fallback": "compatibility_fallback",
        "canonical_validation_legacy_context_planning": "legacy_context_planning_used",
        "canonical_validation_execution_plan_bundle_missing": "execution_plan_bundle_present",
        "canonical_validation_execution_plan_bundle_hash_missing": "execution_plan_bundle_hash",
        "canonical_validation_typed_execution_summary_missing": "typed_execution_summary_present",
        "canonical_validation_execution_summary_hash_missing": "execution_summary_hash",
        "canonical_validation_execution_submit_plan_hash_missing": "execution_submit_plan_hash",
        "canonical_validation_runtime_decision_request_hash_missing": "runtime_decision_request_hash",
        "canonical_validation_runtime_strategy_set_manifest_hash_missing": "runtime_strategy_set_manifest_hash",
        "canonical_validation_research_profile_hash_missing": "research_profile_hash",
        "canonical_validation_market_feature_hash_missing": "market_feature_hash",
        "canonical_validation_final_exit_decision_input_hash_missing": "final_exit_decision_input_hash",
        "canonical_validation_legacy_context_authority": "decision_authority_source",
        "canonical_validation_runtime_replay_planning_error": "runtime_replay_planning_error",
        "canonical_validation_typed_execution_provenance_missing": "execution_evidence_source",
        "canonical_validation_typed_authority_plane_missing": "authority_plane",
        "canonical_validation_artifact_grade_not_validation": "artifact_grade",
        "canonical_validation_rejection_reason_present": "validation_rejection_reason",
        "canonical_validation_legacy_contract_version": "decision_contract_version",
        "canonical_validation_forged_or_unverified_typed_evidence": "typed_execution_evidence",
    }
    return [
        (reason_to_field.get(reason, "validation_provenance"), reason)
...
def runtime_decision_to_canonical_event(
    ...
    *,
    ...
    payload = {
        ...
        "profile_content_hash": profile_content_hash or str(context.get("research_profile_hash") or ""),
        ...
        "research_profile_hash": str(context.get("research_profile_hash") or profile_content_hash or ""),
    }
    for key in (
        "count_basis",
        "kst_day",
    ...
    payload.update(
        {
            ...
            "validation_grade": bool(execution_evidence.get("validation_grade")),
            ...
            "validation_rejection_reason": str(execution_evidence["validation_rejection_reason"]),
        }
    )
...
def research_decision_to_canonical_event(
    ...
    *,
    ...
    payload["research_profile_hash"] = str(payload.get("research_profile_hash") or "")
    ...
    payload["validation_grade"] = bool(payload.get("validation_grade"))
    ...
    payload["validation_rejection_reason"] = str(payload.get("validation_rejection_reason") or "")
    ...
    *,
    ...
    if execution_plan_bundle is not None and execution_plan_bundle.summary is not None:
        ...
        provenance = {
            ...
            "execution_evidence_source": VALIDATION_TYPED_EXECUTION_EVIDENCE_SOURCE,
            "typed_execution_summary_present": True,
            "typed_submit_plan": submit_plan is not None,
            "validation_grade": submit_plan is not None,
            ...
            "validation_rejection_reason": str(observability.get("runtime_replay_planning_error") or ""),
        }
    ...
    if execution_decision and final_signal not in {"BUY", "SELL"}:
        return {
            ...
            "validation_rejection_reason": "context_fallback_execution_evidence",
    ...
    for through_ts_ms in through_ts_list:
        if hasattr(strategy, "decide_runtime_snapshot"):
            ...
            if hasattr(runtime_result, "legacy_strategy_decision"):
                ...
                from bithumb_research.strategy.base import StrategyDecision
