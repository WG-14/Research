    ...
    def _execute(self, prepared: TickPreparation, risk: RiskStageResult) -> ExecutionStageResult:
        ...
        if action in {"BUY", "SELL"}:
            statistical_evidence_policy_required = bool(
                strategy_envelope.provenance.get("statistical_evidence_policy_required")
            )
            policy_drives_execution = bool(statistical_evidence_policy_required)
            try:
                ...
                if statistical_evidence_policy_required or planning_error not in {
            ...
            if (
                bool(strategy_envelope.provenance.get("statistical_evidence_policy_required"))
                ...
                try:
                    planning = self.execution_planner.plan(
                        ExecutionPlanningRequest(
                            ...
                            sellable_qty=prepared.sellable_qty,
                            buy_fraction=self.buy_fraction,
                            statistical_evidence_policy_required=True,
                            allow_execution_compatibility_fallback=bool(
                                strategy_envelope.provenance.get("allow_execution_compatibility_fallback")
                            ),
...
def _build_decision_observability_payload(
    *,
    ...
    try:
        ...
        if not bool(getattr(strategy_plugin, "is_statistical_evidence", False)):
            payload["statistical_evidence"] = False
            payload["validation_extension_missing_reason"] = str(
                getattr(getattr(strategy_plugin, "research_contract", None), "fail_closed_reason", "")
            )
