...
from bithumb_research.lot_model import quantize_to_lot_count
from bithumb_research.quantity_kernel import OrderRuleSnapshot, plan_buy_notional, plan_sell_qty
from bithumb_research.strategy_policy_contract import PositionSnapshot, StrategyDecisionV2
...
VALIDATION_EXECUTION_AUTHORITY_PLANE = "typed_execution_plan_bundle"
VALIDATION_EXECUTION_EVIDENCE_SOURCE = "typed_execution_plan_bundle"
VALIDATION_EXECUTION_ARTIFACT_GRADE = "candidate_validation"
...
def _has_diagnostic_submit_plan_marker(payload: dict[str, object] | None) -> bool:
    ...
    return bool(
        ...
        or payload.get("statistical_evidence") is False
...
class ResearchExecutionPlanBundle:
    ...
    statistical_evidence: bool = True
    recommended_next_action: str = "none"

    @property
    def validation_authoritative(self) -> bool:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "statistical_evidence": bool(self.validation_authoritative),
            "artifact_grade": (
                VALIDATION_EXECUTION_ARTIFACT_GRADE
                if self.validation_authoritative
                else DIAGNOSTIC_EXECUTION_ARTIFACT_GRADE
            ),
            "authority_plane": (
                VALIDATION_EXECUTION_AUTHORITY_PLANE
                if self.validation_authoritative
...
def _research_execution_plan_bundle(
    *,
    ...
    statistical_evidence_required: bool = True,
    block_reason: str = "",
    ...
    if policy_decision is not None:
        ...
        if statistical_evidence_required and missing_typed_submit_plan:
        ...
        return ResearchExecutionPlanBundle(
            ...
            statistical_evidence=not missing_typed_submit_plan,
    ...
    if statistical_evidence_required:
        return ResearchExecutionPlanBundle(
            submit_plan=None,
            summary=None,
            source="research_backtest",
            ...
            execution_engine="research_virtual",
            status="BLOCKED",
            reason_code=block_reason or "validation_requires_typed_execution_submit_plan",
...
def _execution_plan_evidence(plan_bundle: ResearchExecutionPlanBundle | None) -> dict[str, object]:
    from bithumb_research.canonical_decision import canonical_payload_hash
    from bithumb_research.validation_provenance import (
    ...
    if submit_plan is None:
        ...
        validation_authoritative = bool(
            ...
            and plan_bundle.validation_authoritative
        ...
        validation_rejection_reason = (
            ...
            if missing_typed_submit_plan
                ...
                "compatibility_or_diagnostic_execution_evidence_not_statistical_evidence"
                if plan_bundle is not None and not validation_authoritative
                else ""
            )
        )
        return {
            ...
            "artifact_grade": VALIDATION_ARTIFACT_GRADE if validation_authoritative else DIAGNOSTIC_EXECUTION_ARTIFACT_GRADE,
            "authority_plane": VALIDATION_AUTHORITY_PLANE if validation_authoritative else DIAGNOSTIC_EXECUTION_AUTHORITY_PLANE,
            ...
            "statistical_evidence": validation_authoritative,
            "live_authoritative": False,
            "recommended_next_action": (
                "none"
                if validation_authoritative or plan_bundle is None
    ...
    validation_authoritative = bool(plan_bundle.validation_authoritative and not diagnostic_submit_plan)
    return {
        "execution_summary_hash": canonical_payload_hash(summary_payload),
        ...
        "validation_rejection_reason": (
            ""
            if validation_authoritative
            else "compatibility_or_diagnostic_execution_evidence_not_statistical_evidence"
        ),
