...
STATISTICAL_EVIDENCE_WRC = "STATISTICAL_EVIDENCE_WRC"
...
def build_statistical_selection_evidence(
    *,
    ...
    if contract.bootstrap.method == "metric_centered_max_bootstrap":
    ...
    elif _return_panel_supports_validation_wrc(return_panel):
        ...
        method_provenance = (
            {
                "implementation": "bithumb_research.research.statistical_selection.recompute_white_reality_check_block_bootstrap",
    ...
    if contract.bootstrap.method in STATISTICAL_EVIDENCE_METHODS and not statistical_evidence_generation_available:
        gate_reasons = sorted(
            set(gate_reasons)
            | {
                "statistical_evidence_requires_aligned_return_panel",
                "return_panel_statistical_evidence_unavailable",
                "statistical_evidence_statistical_computation_missing",
            }
        )
...
def _extend_statistical_evidence_method_reasons(
    *,
    evidence: dict[str, Any],
    report: dict[str, Any],
    reasons: list[str],
    ...
    if evidence_return_unit not in STATISTICAL_EVIDENCE_RETURN_UNITS:
        reasons.append("statistical_evidence_requires_aligned_return_panel")
    ...
    if isinstance(panel, dict):
        ...
        if panel_return_unit not in STATISTICAL_EVIDENCE_RETURN_UNITS:
            reasons.append("statistical_evidence_requires_aligned_return_panel")
            reasons.append("return_panel_method_support_insufficient")
        if panel.get("statistical_evidence_available") is False:
...
def _statistical_evidence_limitations(
    contract: StatisticalSelectionContract,
    *,
    ...
    limitations = [
        ...
        "aligned_bar_portfolio_return_panel_not_generated",
        "trade_return_panel_cannot_satisfy_statistical_evidence_wrc",
        "official_wrc_generation_requires_aligned_bar_return_panel",
        STATISTICAL_EVIDENCE_GENERATION_UNAVAILABLE_WARNING,
        "spa_not_implemented",
        "deflated_sharpe_not_implemented",
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "missing_observation_policy": (
            ...
            if return_unit in STATISTICAL_EVIDENCE_RETURN_UNITS
...
def _return_panel_supports_validation_wrc(panel: dict[str, Any] | None) -> bool:
    return (
        isinstance(panel, dict)
        and panel.get("return_unit") in STATISTICAL_EVIDENCE_RETURN_UNITS
...
def recompute_white_reality_check_block_bootstrap(
    *,
    ...
    if panel.get("return_unit") not in STATISTICAL_EVIDENCE_RETURN_UNITS:
        return None
    if panel.get("statistical_evidence_available") is False:
        return None
