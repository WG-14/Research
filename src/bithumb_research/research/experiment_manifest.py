...
from bithumb_research.execution_reality_contract import (
    evaluate_execution_reality_policy,
    unsupported_capability_reasons,
)
from bithumb_research.market_regime import RegimeAcceptanceGate
...
class ScenarioCostAssumption:
    ...
    validation_eligible_as_base: bool = False
    ...
    def as_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "validation_eligible_as_base": self.validation_eligible_as_base,
            "source": self.source,
        }
...
class AcceptanceGate:
    ...
    final_holdout_required_for_validation: bool = True
    ...
    def as_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "final_holdout_required_for_validation": self.final_holdout_required_for_validation,
...
def parse_manifest(payload: dict[str, Any]) -> ExperimentManifest:
    ...
    if unknown:
        raise ManifestValidationError(f"unknown manifest field(s): {','.join(unknown)}")
    ...
    cost_policy_reasons = validation_cost_assumption_policy_reasons(
        research_classification=research_classification,
        execution_model=execution_model,
    )
...
def _parse_research_classification(value: Any) -> str:
    try:
        return normalize_research_classification(value)
    ...
    *,
    ...
    return ExecutionScenario(
        ...
        cost_assumption=_scenario_cost_assumption(
            ...
            validation_eligible_as_base=raw.get("validation_eligible_as_base"),
            source="execution_model",
        ),
    )
...
def _scenario_cost_assumption(
    *,
    ...
    slippage_source: str,
    valid_for: Any,
    validation_eligible_as_base: Any,
    source: str,
...
def validation_cost_assumption_policy_reasons(
    *,
    research_classification: str,
    execution_model: ExecutionModelConfig,
) -> list[str]:
    ...
    if execution_model.source == "legacy_cost_model":
        reasons.append("validation_legacy_cost_model_not_validation_eligible")
    ...
    if not base_assumptions:
        reasons.append("validation_base_cost_assumption_required")
    if scenarios and all(scenario.scenario_role == "stress" for scenario in scenarios):
        reasons.append("validation_stress_only_cost_model_not_validation_eligible")
    for assumption in base_assumptions:
        ...
        if not assumption.slippage_source:
            reasons.append("validation_cost_assumption_source_required")
        if assumption.role == "stress" or not assumption.validation_eligible_as_base:
            reasons.append("validation_stress_only_cost_model_not_validation_eligible")
...
def _parse_execution_timing(value: Any) -> ExecutionTimingPolicy:
    ...
    return ExecutionTimingPolicy(
        ...
        min_execution_reality_level_for_validation=min_level,
    ...
    *,
    ...
    diagnostic = ExecutionScenario(
        ...
        cost_assumption=ScenarioCostAssumption(
            ...
            fee_authority_policy="diagnostic_only_excluded_from_validation",
            slippage_bps=0.0,
...
def _parse_acceptance_gate(payload: dict[str, Any]) -> AcceptanceGate:
    allowed_fields = {
        ...
        "final_holdout_required_for_validation",
...
def _parse_statistical_validation(
    ...
    *,
    ...
    if validation_required and primary_metric == "sharpe_like":
        raise ManifestValidationError(
            "statistical_validation.primary_metric sharpe_like is not allowed for validation-bound manifests "
...
def _parse_research_run(value: Any) -> ResearchRunPolicy:
    ...
    if run_purpose not in {
        ...
        raise ManifestValidationError(
            ...
            "strategy_performance_diagnostic, validation_lifecycle, or validation_evidence"
        )
...
def _parse_research_audit_trail(value: Any) -> ResearchAuditTrailPolicy:
    ...
    return ResearchAuditTrailPolicy(
        ...
        required_for_validation=bool(value.get("required_for_validation", True)),
    )
