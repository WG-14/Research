...
CANONICAL_EQUIVALENCE_FIELDS_V2 = tuple(
    ...
    not in {
        ...
        # validation-equivalence authority.
        "decision_ts",
...
def compare_decision_export_artifacts(
    *,
    ...
    if artifact_binding:
        report["validation_grade_comparison"] = False
        report["ok"] = False
...
VALIDATION_GRADE_DECISION_EQUIVALENCE_REQUIRED_TRUE_FIELDS = (
    "ok",
    "validation_grade_comparison",
    "canonical_schema",
    "canonical_v2_schema",
...
def validation_grade_decision_equivalence_fail_reasons(report: dict[str, Any]) -> tuple[str, ...]:
    """Return fail-closed reasons when a report is not validation-grade v2 evidence."""
    reasons: list[str] = []
    reasons.extend(f"decision_equivalence_{reason}" for reason in evidence_rejection_reasons(report))
    for field in VALIDATION_GRADE_DECISION_EQUIVALENCE_REQUIRED_TRUE_FIELDS:
...
def decision_equivalence_report_semantic_fail_reasons(report: dict[str, Any]) -> tuple[str, ...]:
    """Return fail-closed reasons for source validation/profile evidence use."""
    reasons: list[str] = []
    ...
    if report.get("validation_grade_comparison") is not True:
        reasons.append("decision_equivalence_status_not_pass")
    ...
    *,
    ...
    if expected_hash is not None:
        ...
        if not expected.startswith("sha256:"):
            ...
            reasons.append("decision_equivalence_report_hash_mismatch")
    reasons.extend(validation_grade_decision_equivalence_fail_reasons(report))
    return tuple(sorted(set(reasons)))


def require_validation_grade_decision_equivalence(report: dict[str, Any]) -> None:
    reasons = validation_grade_decision_equivalence_fail_reasons(report)
    if reasons:
        raise ValueError("decision_equivalence_not_validation_grade:" + ",".join(reasons))
...
def load_decision_export_artifact(
    ...
    *,
    ...
    if payload.get("validation_grade_export") is not True:
        raise ValueError("decision_export_not_validation_grade")
    ...
    for source, decisions in (("research", research_decisions), ("runtime", runtime_decisions)):
        for item in decisions:
            ...
            result = validate_canonical_decision_payload(
                item,
                validation_grade=is_canonical_decision_v2(item),
                require_validation_provenance=state_class != "flat_no_dust_no_position",
            )
...
def _claims_scope(
    *,
    ...
    return {
        ...
        "validation_claim": "positive_decision_equivalence_for_explicitly_modeled_state_classes_only",
...
def _recommended_next_action(
    *,
    ...
    if state_coverage_matrix and any(
        ...
        return "extend_research_lot_native_position_model_before_claiming_lifecycle_equivalence"
    return "inspect_research_runtime_decision_drift_before_validation"
