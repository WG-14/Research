...
def build_research_readiness_report(
    *,
    ...
    failed = False
    for split_name in _split_names(manifest):
        snapshot = load_dataset_split(
            db_path=resolved_db_path,
            manifest=manifest,
            split_name=split_name,
        )
        report = build_dataset_quality_report(db_path=resolved_db_path, snapshot=snapshot).payload
    ...
    walk_forward = _walk_forward_payload(manifest)
...
def _split_names(manifest: ExperimentManifest) -> tuple[str, ...]:
...
def _walk_forward_payload(manifest: ExperimentManifest) -> dict[str, Any]:
