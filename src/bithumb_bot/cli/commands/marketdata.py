...
def _backfill_candles(args: argparse.Namespace, _context) -> int:
    ...
    def _print_progress(progress):
        ...
        print(
            ...
            f"written={progress.written_count} upserted_count={progress.upserted_count} "
            f"new_bucket_count={progress.new_bucket_count} replaced_bucket_count={progress.replaced_bucket_count} "
            f"recovered_missing_bucket_count={progress.recovered_missing_bucket_count} "
            f"remaining_missing_bucket_count={progress.remaining_missing_bucket_count} "
            f"duplicate_pages={progress.duplicate_page_count} "
            ...
            f"api_fetch_status={progress.api_fetch_status} cursor_status={progress.cursor_status} "
            f"db_write_status={progress.db_write_status} coverage_status={progress.coverage_status}"
        )
    ...
    try:
        ...
        print(
            "[BACKFILL-CANDLES] "
            f"api_fetch_status=ERROR cursor_status=OK db_write_status=ERROR coverage_status=INCOMPLETE "
            f"source_gap_status=API_UNAVAILABLE_CANDIDATE research_readiness_status=NOT_EVALUATED "
            f"operator_result_code=API_FETCH_ERROR operator_result_reason=api_fetch_error error={exc}"
        )
        return 1
    ...
    print(
        ...
        f"written={result.progress.written_count} upserted_count={result.progress.upserted_count} "
        f"new_bucket_count={result.progress.new_bucket_count} replaced_bucket_count={result.progress.replaced_bucket_count} "
        f"recovered_missing_bucket_count={result.progress.recovered_missing_bucket_count} "
        f"remaining_missing_bucket_count={result.progress.remaining_missing_bucket_count} "
        f"cursor_fallbacks={result.progress.cursor_fallback_count} "
        f"status={result.progress.status} reason={result.progress.reason or 'none'} "
        f"api_fetch_status={result.api_fetch_status} cursor_status={result.cursor_status} "
        f"db_write_status={result.db_write_status} coverage_status={result.coverage_status} "
        f"source_gap_status={result.source_gap_status} research_readiness_status={result.research_readiness_status} "
        f"operator_result_code={result.operator_result_code} operator_result_reason={result.operator_result_reason} "
        f"request_interval_ms={result.request_interval_ms} max_retries={result.max_retries} "
        f"rate_limit_policy={result.rate_limit_policy}"
    )
    ...
    if result.progress.status != "COMPLETE":
        print(
            "[BACKFILL-CANDLES] result=BACKFILL_INCOMPLETE "
            f"reason={result.operator_result_reason} "
            f"api_fetch_status={result.api_fetch_status} cursor_status={result.cursor_status} "
            f"db_write_status={result.db_write_status} coverage_status={result.coverage_status} "
            f"operator_result_code={result.operator_result_code} operator_result_reason={result.operator_result_reason} "
            "next_action=inspect backfill progress; if coverage remains incomplete run "
            "research-missing-candles -> retry-missing-candles -> probe-missing-candles -> "
            "classify-persistent-missing-candles -> research-readiness --manifest <manifest>"
        )
        return 1
    if not coverage_complete:
        print(
            f"[BACKFILL-CANDLES] result={result.operator_result_code} "
            f"reason={result.operator_result_reason} "
            f"api_fetch_status={result.api_fetch_status} cursor_status={result.cursor_status} "
            f"db_write_status={result.db_write_status} coverage_status={result.coverage_status} "
            f"operator_result_code={result.operator_result_code} operator_result_reason={result.operator_result_reason} "
            "next_action=run research-missing-candles -> retry-missing-candles -> "
            "probe-missing-candles -> classify-persistent-missing-candles -> "
            "research-readiness --manifest <manifest>"
        )
