...
RESEARCH_NOTIFICATION_POLICIES = frozenset({"best_effort", "require_delivery", "disabled"})
...
class ResearchSettings:
    ...
    def from_env(cls) -> "ResearchSettings":
        ...
        notification_policy = os.getenv("RESEARCH_NOTIFICATION_POLICY", "disabled").strip().lower() or "disabled"
        if notification_policy not in RESEARCH_NOTIFICATION_POLICIES:
            allowed = ", ".join(sorted(RESEARCH_NOTIFICATION_POLICIES))
            raise ResearchSettingsError(
                "RESEARCH_NOTIFICATION_POLICY must be one of " f"{allowed}"
            )
        return cls(
            data_root=data_root,
