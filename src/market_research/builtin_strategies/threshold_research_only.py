...
THRESHOLD_RESEARCH_ONLY_SPEC = StrategySpec(
    ...
    rule_spec=StrategyRuleSpec(1,
        entry=StrategyRuleDeclaration("close_above_threshold", "Buy when close is strictly above threshold.",
            "close > THRESHOLD_CLOSE_ABOVE", ("THRESHOLD_CLOSE_ABOVE",)),
        ...
        entry_prohibitions=(StrategyRuleDeclaration("existing_or_pending_position", "Block duplicate entry.", "position or buy pending"),)),
    feature_definitions=(StrategyFeatureDefinition(
        "close", "Current completed candle close used by the research threshold.",
        ("candles",), "current_candle.close", (),
    ),))
