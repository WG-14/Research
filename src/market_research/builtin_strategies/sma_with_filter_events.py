...
def build_sma_with_filter_research_events(
    *,
    ...
    for index in range(long_n, len(closes)):
        ...
        curr_s, curr_l = (
            _sma(closes, index + 1, short_n),
            _sma(closes, index + 1, long_n),
        )
        raw = (
            "BUY"
        ...
        over_window = max(
            1, int(parameter_values.get("SMA_FILTER_OVEREXT_LOOKBACK") or 3)
        )
        ...
        features = {
            ...
            "short_sma": curr_s,
            "long_sma": curr_l,
            "prev_short_sma": prev_s,
            "prev_long_sma": prev_l,
            "gap_ratio": gap,
            "volatility_ratio": volatility,
            "overextended_ratio": overextended,
        }
