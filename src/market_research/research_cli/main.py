...
def build_parser(
    registry: dict[str, ResearchCommandSpec] | None = None,
) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="market-research")
    subparsers = parser.add_subparsers(dest="cmd", required=False)
    for spec in sorted(
        (registry or command_registry()).values(), key=lambda item: item.name
    ):
...
def main(
    argv: list[str] | None = None, context: ResearchAppContext | None = None
) -> int:
