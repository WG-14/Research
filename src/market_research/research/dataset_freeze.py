...
def canonical_candle_rows_hash(
    rows: list[tuple[Any, ...]], *, market: str, interval: str
) -> str:
...
def sqlite_candles_schema_hash(db_path: str | Path) -> str:
    ...
    try:
        ...
        index_list = [
            tuple(row) for row in conn.execute("PRAGMA index_list(candles)").fetchall()
        ]
        return artifact_schema_hash(
            {"table": "candles", "table_info": table_info, "index_list": index_list}
        )
    finally:
        conn.close()
...
def freeze_sqlite_candles_dataset(
    *,
    ...
    staging = Path(
        tempfile.mkdtemp(prefix=f".{digest}.staging-", dir=artifact_dir.parent)
    )
    ...
    try:
        ...
        final_manifest = build_artifact_manifest(
            ...
            end_ts=int(end_ts),
            coverage_start_ts=int(start_ts),
    ...
    *,
    ...
    try:
        ...
        if str(exc) in {
            ...
            "existing_artifact_schema_conflict",
            "artifact_scope_verification_failed",
        }:
            raise
    ...
    *,
    ...
    if actual_schema != manifest.schema_hash:
        raise DatasetFreezeError(
            "existing_artifact_schema_conflict"
            if committed
            else "artifact_schema_hash_verification_failed"
        )
    ...
    if actual_pairs != {(market, interval)} or (manifest.market, manifest.interval) != (
        market,
        interval,
    ):
        raise DatasetFreezeError("artifact_market_interval_verification_failed")
    actual_scope = (
        min((int(row[2]) for row in rows), default=int(start_ts)),
        max((int(row[2]) for row in rows), default=int(end_ts)),
    )
    actual_coverage = (
        actual_scope[0],
        _coverage_end_ts(end_ts=actual_scope[1], interval=interval),
    )
    if (
        actual_scope != (int(start_ts), int(end_ts))
...
def _is_destination_conflict(exc: OSError) -> bool:
    ...
    return isinstance(exc, FileExistsError) or exc.errno in {
        errno.EEXIST,
        errno.ENOTEMPTY,
    }
...
def _result(
    ...
    return {
        ...
        "artifact_manifest_uri": str(manifest_path),
...
def _read_candle_rows(
    db_path: str | Path, *, market: str, interval: str, start_ts: int, end_ts: int
) -> list[tuple[Any, ...]]:
    conn = sqlite3.connect(
        f"file:{Path(db_path).expanduser().resolve()}?mode=ro", uri=True
    )
    try:
        return conn.execute(
            "SELECT ts, open, high, low, close, volume FROM candles WHERE pair=? AND interval=? AND ts>=? AND ts<=? ORDER BY ts ASC",
...
def _read_all_artifact_rows(db_path: str | Path) -> list[tuple[Any, ...]]:
    ...
    try:
        return conn.execute(
            "SELECT pair, interval, ts, open, high, low, close, volume FROM candles ORDER BY pair, interval, ts"
        ).fetchall()
    finally:
        conn.close()
