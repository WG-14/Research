...
def parse_benchmark_suite_contract(
    ...
    *,
    ...
    schema_version = _positive_int(value.get("schema_version"), "benchmark_suite.schema_version")
    if schema_version != 1:
    ...
    if validation_required and not required:
        raise ValueError("benchmark_suite.required_for_validation must be true for validation-bound manifests")
    contract = BenchmarkSuiteContract(
        ...
        same_holding_period=_parse_same_holding_period(value.get("same_holding_period")),
        simpler_strategy=_parse_strategy_reference(
            value.get("simpler_strategy"),
            field="benchmark_suite.simpler_strategy",
            registry=registry,
        ),
        approved_strategy=_parse_approved_strategy(value.get("approved_strategy"), registry=registry),
    )
    if candidate_strategy_name and contract.simpler_strategy.strategy_name == candidate_strategy_name:
        raise ValueError("benchmark_suite.simpler_strategy must differ from the candidate strategy")
    return contract
...
def _parse_random_entry(value: Any) -> RandomEntryBenchmarkContract:
    ...
    iterations = _positive_int(value.get("iterations"), "benchmark_suite.random_entry.iterations")
...
def _parse_strategy_reference(value: Any, *, field: str, registry: Any) -> StrategyBenchmarkReference:
    if not isinstance(value, dict):
    ...
    if not name or not version or not isinstance(parameters, dict):
        raise ValueError(f"{field} requires strategy_name, strategy_version, and parameter_values")
    ...
    if unknown_parameters:
        raise ValueError(f"{field}.parameter_values unsupported fields: {','.join(unknown_parameters)}")
    missing = sorted(
    ...
    if missing:
        raise ValueError(f"{field}.parameter_values missing required fields: {','.join(missing)}")
    return StrategyBenchmarkReference(name, version, dict(parameters))


def _parse_approved_strategy(value: Any, *, registry: Any) -> ApprovedStrategyBenchmarkReference:
    if not isinstance(value, dict):
    ...
    strategy = _parse_strategy_reference(
        {key: value.get(key) for key in ("strategy_name", "strategy_version", "parameter_values")},
    ...
    if not path or not resolved_path.is_absolute():
        raise ValueError("benchmark_suite.approved_strategy.approval_artifact_path must be absolute")
    ...
    if _SHA256_RE.fullmatch(content_hash) is None:
        raise ValueError("benchmark_suite.approved_strategy.approval_artifact_hash must be sha256:<64 hex>")
