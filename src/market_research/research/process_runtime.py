...
def resolve_research_process_runtime(
    *,
    ...
    requested = _normalize_requested_start_method(
        os.environ.get("RESEARCH_RESEARCH_MP_START_METHOD") or requested_start_method,
    ...
    *,
    ...
    if requested in {"auto_safe", "auto"}:
        ...
        raise ResearchProcessRuntimeError(
            ...
            "set RESEARCH_RESEARCH_ALLOW_UNSAFE_FORK=1 only for diagnostic fork runs"
        )
    ...
    if requested == "fork" and not unsafe_fork_allowed:
        raise ResearchProcessRuntimeError(
            ...
            "RESEARCH_RESEARCH_ALLOW_UNSAFE_FORK=1 for diagnostic use"
        )
...
def _resolve_worker_budget(requested_workers: int) -> tuple[int, dict[str, object]]:
    caps: list[int] = []
    env_cap_raw = os.environ.get("RESEARCH_RESEARCH_MAX_WORKERS")
    if env_cap_raw:
    ...
    if total_budget_raw:
        total_budget = _positive_int(total_budget_raw, "RESEARCH_TOTAL_PROCESS_BUDGET")
...
def _process_budget_metadata(
    ...
    *,
    ...
    outer_worker_count = _outer_worker_count()
    total_budget_raw = os.environ.get("RESEARCH_TOTAL_PROCESS_BUDGET")
    research_cap_raw = os.environ.get("RESEARCH_RESEARCH_MAX_WORKERS")
    batch_child_cap_raw = os.environ.get("RESEARCH_BATCH_CHILD_WORKER_BUDGET")
    return {
        "schema_version": 1,
        "outer_parallel_context": _outer_parallel_context(),
