...
class DatasetVerificationResult:
    ...
    def __post_init__(self) -> None:
        ...
        for status, expected, actual, label in (
            ...
            if status in {VerificationStatus.VERIFIED, VerificationStatus.MISMATCH}:
                ...
                if status is VerificationStatus.VERIFIED and expected != actual:
                    raise ValueError(
                        f"dataset_verification_{label}_verified_hash_mismatch"
                    )
                if status is VerificationStatus.MISMATCH and expected == actual:
                    raise ValueError(
                        f"dataset_verification_{label}_mismatch_hash_equal"
                    )
        components = (
            self.content_status,
            self.schema_status,
        ...
        if (
            any(value is VerificationStatus.MISMATCH for value in components)
            and self.overall_status is not VerificationStatus.MISMATCH
        ):
            raise ValueError(
                "dataset_verification_component_mismatch_requires_overall_mismatch"
            )
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            key: canonical_mutable(
                value.value if isinstance(value, VerificationStatus) else value
            )
    ...
    def require_verified(self) -> None:
        if self.overall_status is not VerificationStatus.VERIFIED:
            raise ValueError(
                f"dataset_verification_not_verified:{self.overall_status.value}"
            )


def verification_allowed(
    *, classification: str, result: DatasetVerificationResult
) -> bool:
