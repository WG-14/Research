...
def _validate_fill_timeline(fill: ExecutionFill) -> None:
    ...
    if not (int(fill.decision_ts) <= int(fill.submit_ts_assumption) <= int(fill.fill_reference_ts) <= effective):
...
def run_common_simulation_backtest(
    *, plugin: ResearchStrategyPlugin, dataset: DatasetSnapshot, parameter_values: dict[str, Any], fee_rate: float,
    ...
    pending_buy = False
    open_entry: tuple[int, float, float] | None = None
        ...
        for fill in sorted(tuple(pending), key=lambda item: (int(item.portfolio_effective_ts or 0), item.fill_id)):
            ...
            if fill.side == "BUY":
                ...
                if open_entry is None:
                    open_entry = (int(fill.portfolio_effective_ts or boundary), float(fill.avg_fill_price or 0.0), float(fill.filled_qty))
                ...
                entry_ts, entry_price, _ = open_entry
                pnl = float(entry.realized_pnl or 0.0)
                closed.append(ClosedTradeRecord(exit_ts=int(entry.effective_ts), entry_ts=entry_ts, net_pnl=pnl, entry_price=entry_price, exit_price=float(fill.avg_fill_price or 0.0), fee_total=ledger.fee_total, slippage_total=ledger.slippage_total))
    ...
    for event in events:
        ...
        decisions.append({"decision_id": decision_id, "candle_ts": event.candle_ts, "decision_ts": event.decision_ts,
            "strategy_name": event.strategy_name, "raw_signal": event.raw_signal, "entry_signal": event.entry_signal,
            "exit_signal": event.exit_signal, "final_signal": event.final_signal, "reason": event.reason,
            "feature_snapshot": event.feature_snapshot, "strategy_diagnostics": event.strategy_diagnostics})
        ...
        if intent is not None:
            ...
            request = ExecutionRequest(signal_ts=signal.signal_candle_start_ts, decision_ts=signal.decision_ts, side=intent.side,
            ...
            fill = replace(fill, request_id=request_id, fill_id=sha256_prefixed({"request_id": request_id, "model": fill.model_params_hash, "status": fill.fill_status, "filled_qty": fill.filled_qty, "price": fill.avg_fill_price}), portfolio_effective_ts=fill.fill_reference_ts)
    ...
    summary.update({"execution_request_count": len(requests), "execution_model_invocation_count": sum(1 for item in requests if item.fill_reference_price is not None), "fill_count": len(fills), "declared_execution_timing_hash": sha256_prefixed(timing.as_dict()), "executed_execution_timing_hash": sha256_prefixed(timing.as_dict()), "declared_execution_model_hash": model_params_hash(model.params_payload()), "executed_execution_model_hash": model_params_hash(model.params_payload()), "execution_request_stream_hash": _stream_hash(tuple(requests)), "execution_fill_stream_hash": _stream_hash(tuple(fills)), "portfolio_ledger_hash": _stream_hash(ledger_entries), "timing_invariant_status": timing_status})
