...
def build_research_decision_report(
    *,
    ...
    report = {
        ...
        "selected_candidate_id": selected.get("parameter_candidate_id")
        ...
        "sections": {
            "hypothesis_and_experiment_conditions": {
                ...
                "execution_timing_policy": selection_report.get(
                    "execution_timing_policy"
                ),
                "portfolio_policy": selection_report.get("portfolio_policy"),
                "risk_policy": selection_report.get("risk_policy")
                or getattr(manifest, "risk_policy").as_dict(),
            },
            "data_quality": {
                "status": selection_report.get("dataset_quality_gate_status"),
                "reasons": list(
                    selection_report.get("dataset_quality_gate_reasons") or []
                ),
            ...
            "core_performance": {
                ...
                or selected.get("validation_metrics")
                or {},
                "validation_metrics_v2": primary.get("validation_metrics_v2")
                or selected.get("validation_metrics_v2")
                or {},
            ...
            "trade_analysis": {
                "closed_trade_diagnostics": selection_report.get(
                    "closed_trade_diagnostics_summary"
                )
                or {},
                "execution_event_summary": primary.get("execution_event_summary")
                or selected.get("execution_event_summary")
                or {},
                "participation_summary": selected.get("participation_summary") or {},
            },
            "cost_analysis": {
                "cost_assumption_contract": selection_report.get(
            ...
            "market_regime_analysis": {
                ...
                "bucket_performance": selection_report.get(
                ...
                "allowed_regimes": list(
                    selection_report.get("allowed_live_regimes") or []
                ),
                "blocked_regimes": list(
                    selection_report.get("blocked_live_regimes") or []
                ),
            },
            "parameter_robustness": {
                "stress_suite": selected.get("validation_stress_suite")
                or selection_report.get("best_validation_stress_suite")
                or {},
                "stress_gate_result": selection_report.get("stress_suite_gate_result"),
                "stress_fail_reasons": list(
                    selection_report.get("stress_suite_fail_reasons") or []
                ),
                "statistical_gate_result": selection_report.get(
                    "statistical_gate_result"
                ),
                "statistical_fail_reasons": list(
                    selection_report.get("statistical_gate_fail_reasons") or []
                ),
                "walk_forward_gate_result": selection_report.get(
                    "walk_forward_gate_result"
                ),
                "walk_forward_metrics": selected.get("walk_forward_metrics") or {},
                "candidate_final_scores": selection_report.get("candidate_final_scores")
                or [],
            },
            "out_of_sample_results": {
                "selection_artifact_hash": selection_report.get(
            ...
            "known_limitations": {
                ...
                "statistical": list(
                    selection_report.get("statistical_evidence_limitations") or []
                ),
                "stress": _stress_limitations(selected, selection_report),
            },
            "research_conclusion": {
                "automated_evidence_conclusion": _automated_conclusion(
                    validation_result
                ),
                "validation_result": validation_result,
    ...
    if reasons:
        raise ResearchDecisionReportError(
            "research_decision_report_invalid:" + ",".join(reasons)
        )
    report["content_hash"] = sha256_prefixed(
        content_hash_payload(report), label="research_decision_report"
    )
    return report


def validate_research_decision_report(
    report: object, *, verify_hash: bool = True
) -> list[str]:
    ...
    if (
        ...
        or report.get("artifact_type") != "research_decision_report"
    ):
...
def _primary_scenario(selected: dict[str, Any]) -> dict[str, Any]:
    ...
    return next(
        (
            item
            for item in selected.get("scenario_results")
            or selected.get("scenarios")
            or []
            if isinstance(item, dict)
            and str(item.get("scenario_id") or "") == primary_id
        ),
        {},
    )
...
def _holdout_result(
    confirmation: dict[str, Any] | None, selected: dict[str, Any]
) -> dict[str, Any]:
...
def _failure_periods(
    selection_report: dict[str, Any], selected: dict[str, Any]
) -> dict[str, Any]:
    ...
    return {
        ...
        "period_ablation": stress.get("period_ablation")
        if isinstance(stress, dict)
        else None,
        "walk_forward_windows": walk_forward.get("windows")
...
def _stress_limitations(
    selected: dict[str, Any], selection_report: dict[str, Any]
) -> list[str]:
