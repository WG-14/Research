...
def allocate_batch_child_process_budget(*, max_concurrent_manifests: int) -> dict[str, Any]:
    ...
    child_env: dict[str, str] = {"RESEARCH_RESOURCE_PLAN_SOURCE": "research_batch"}
    if total_budget is not None:
        child_env["RESEARCH_TOTAL_PROCESS_BUDGET"] = str(int(total_budget))
    if child_budget is not None:
        child_env["RESEARCH_RESEARCH_MAX_WORKERS"] = str(child_budget)
        child_env["RESEARCH_BATCH_CHILD_WORKER_BUDGET"] = str(child_budget)
    return {
        "schema_version": 1,
