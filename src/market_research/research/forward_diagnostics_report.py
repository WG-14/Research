...
def forward_diagnostics_report_paths(
    *,
    ...
    return ForwardDiagnosticsReportPaths(
        report_path=base_data_dir
        / "reports"
        / "research"
        / experiment_id
        / "forward_diagnostics_report.json",
...
def write_forward_diagnostics_report(
    *,
    ...
    if not isinstance(result, ForwardDiagnosticsResult):
        raise TypeError(
            "write_forward_diagnostics_report requires ForwardDiagnosticsResult"
        )
    validate_forward_diagnostics_split_policy(result)
    paths = forward_diagnostics_report_paths(
        manager=manager, experiment_id=manifest.experiment_id
    )
    _write_bucket_metrics_csv(
        paths.feature_bucket_metrics_path,
        [metric.as_dict() for metric in result.feature_bucket_metrics],
    )
    _write_horizon_metrics_csv(
        paths.feature_horizon_metrics_path,
        [metric.as_dict() for metric in result.feature_horizon_metrics],
    )
    ...
    report = apply_artifact_contract(
        {
            ...
            "feature_provider_specs": [
                spec.as_report_dict() for spec in result.feature_provider_specs
            ],
            ...
            "feature_bucket_metrics_hash": _file_hash(
                paths.feature_bucket_metrics_path
            ),
            "feature_horizon_metrics_hash": _file_hash(
                paths.feature_horizon_metrics_path
            ),
