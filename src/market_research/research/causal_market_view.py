...
class CausalMarketView:
    ...
    def __post_init__(self) -> None:
        ...
        quotes = tuple(q for q in snapshot.execution_top_of_book_quotes()
                       if int(q.ts) <= self.decision_boundary_ts)
        depths = tuple(d for d in snapshot.orderbook_depth_snapshots
                       if int(d.ts) <= self.decision_boundary_ts)
        bounded = replace(snapshot, candles=candles,
    ...
    @classmethod
    def from_dataset(cls, dataset: DatasetSnapshot, current_index: int,
                     decision_boundary_ts: int) -> "CausalMarketView":
        """Copy only observable rows into the strategy-visible object graph."""
        candles = dataset.candles[:current_index + 1]
        quotes = tuple(q for q in dataset.execution_top_of_book_quotes()
                       if int(q.ts) <= decision_boundary_ts)
        depths = tuple(d for d in dataset.orderbook_depth_snapshots
                       if int(d.ts) <= decision_boundary_ts)
        aligned = dataset.top_of_book_quotes[:current_index + 1]
        bounded = replace(dataset, candles=candles, top_of_book_quotes=aligned,
