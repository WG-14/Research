...
def load_candidate_regime_policy_from_path(
    path: str | Path | None,
...
def normalize_live_regime_policy(
    ...
    source = str(
        candidate_policy.get("_policy_source")
        or candidate_policy.get("policy_source")
        or "injected"
    )
    ...
    allowed_present, allowed = _list_field(
        payload, "allowed_live_regimes", "allowed_regimes"
    )
    ...
    blocked_present, blocked = _list_field(
        payload, "blocked_live_regimes", "blocked_regimes"
    )
...
def _row_regime(row: Any) -> str:
    return str(
        ...
        else getattr(row, "regime", "unknown")
    )
...
def _row_dimension(row: Any) -> str:
    return str(
        row.get("dimension")
        if isinstance(row, dict)
        else getattr(row, "dimension", "unknown")
    )
...
def _matches(row: Any, regime: str) -> bool:
    ...
    return name == regime or (
        dimension
        in {"price_regime", "volatility_bucket", "volume_bucket", "liquidity_bucket"}
        and name == regime
    )
    ...
    *,
    ...
    if not gate.required:
        optional_evidence = {
            _row_regime(row): {
                ...
                "profit_factor_unbounded": _row_value(
                    row, "profit_factor_unbounded", False
                )
                is True,
    ...
    for row in performance_rows:
        ...
        evidence[_row_regime(row)] = {
            "trade_count": int(_row_value(row, "trade_count", 0) or 0),
            "profit_factor": _row_value(row, "profit_factor"),
            "profit_factor_unbounded": _row_value(row, "profit_factor_unbounded", False)
            is True,
            "expectancy": _row_value(row, "expectancy"),
            "net_pnl": float(_row_value(row, "net_pnl", 0.0) or 0.0),
    ...
    for regime, min_pf in gate.min_profit_factor_by_regime.items():
        ...
        bounded_rows = [
            row
            for row in rows
            if _row_value(row, "profit_factor_unbounded", False) is not True
        ]
        pf_values = [
        ...
        if not pf_values or min(pf_values) < float(min_pf):
            ...
            reasons.append(
                f"regime_gate_failed: {regime} profit_factor={actual} < min={float(min_pf)}"
            )

    for regime, min_expectancy in gate.min_expectancy_by_regime.items():
        rows = [row for row in performance_rows if _matches(row, regime)]
        values = [
            float(_row_value(row, "expectancy"))
            for row in rows
            if _row_value(row, "expectancy") is not None
        ]
        if not values or min(values) < float(min_expectancy):
            reasons.append(
                f"regime_gate_failed: {regime} expectancy={min(values) if values else None} < min={float(min_expectancy)}"
            )

    composite_rows = [
        row for row in performance_rows if _row_dimension(row) == "composite_regime"
    ]
    losses = [
        abs(float(_row_value(row, "net_pnl", 0.0) or 0.0))
        for row in composite_rows
        if float(_row_value(row, "net_pnl", 0.0) or 0.0) < 0.0
    ]
    if losses and gate.max_loss_share_by_single_regime is not None:
        ...
        if share > float(gate.max_loss_share_by_single_regime):
            reasons.append(
                f"regime_loss_concentration_failed: max_loss_share={share:.6f}"
            )
    profits = [
        float(_row_value(row, "net_pnl", 0.0) or 0.0)
        for row in composite_rows
        if float(_row_value(row, "net_pnl", 0.0) or 0.0) > 0.0
    ]
    if profits and gate.max_pnl_dependency_by_single_regime is not None:
        ...
        if share > float(gate.max_pnl_dependency_by_single_regime):
            reasons.append(
                f"regime_profit_dependency_failed: max_pnl_dependency={share:.6f}"
            )
