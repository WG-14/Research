...
- Strategy Level: one of `level_1_research_only`, `level_2_replay_compatible`, `level_3_validation_grade`, or `not_strategy_related`.
...
- Entry Point Group: `bithumb_research.strategy_plugins` or `not_applicable`.
...
- [ ] In-repo built-in strategy plugins updated `src/bithumb_research/strategy_plugins/builtin_manifest.py` with a valid Built-in Reason, or external strategy packages declared a `bithumb_research.strategy_plugins` entry point.
- [ ] Strategy appears in `list_research_strategy_plugins()` and resolves through `resolve_research_strategy_plugin()`.
- [ ] Strategy plugin inventory verification: `uv run bithumb-research strategy-plugin-inventory --json` shows the expected strategy source, contract hash, live eligibility, and fail-closed reason.
- [ ] Strategy plugin validate verification: `uv run bithumb-research strategy-plugin-validate --strategy <name> --target <target> --json` checked target verdicts where applicable.
